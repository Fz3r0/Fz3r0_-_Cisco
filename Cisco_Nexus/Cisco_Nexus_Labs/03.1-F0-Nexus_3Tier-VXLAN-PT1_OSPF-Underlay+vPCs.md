...
## 🔀 `SWITCH-L2` - (Switch Layer 2 - WAN Switch) 
...
exit
#! Static default route (Default Gateway) @ MGMT-SWITCH
...
!#######################################################################################
!#  TRUNK INTERFACES (VRF: management) ==>> @ LAN                                      #
!#######################################################################################

!# INTERFACE :: ETH 0/2 @ SWITCH-L2 (Access) :: VRF management
int ethernet0/2
   description ** Trunk @ VRF : management **
   no shutdown
   duplex full
      switchport
...
!#######################################################################################
!#  TRUNK INTERFACES (VRF: default) ==>> @ WAN                                         #
!#######################################################################################

!# INTERFACE :: ETH 1/3 @ SWITCH-L2 (Trunk) :: VRF DEFAULT (@ WAN - GATEWAY)
int ethernet1/3
   description ** Trunk @ WAN (DEFAULT VRF) **
   no shutdown
   duplex full
      switchport
      switchport trunk encapsulation dot1q
...
!#######################################################################################
!#  TRUNK INTERFACES (VRF: management) ==>> @ WAN                                      #
!#######################################################################################

!# INTERFACE :: ETH 1/2 @ SWITCH-L2 (Trunk) :: VRF MANAGEMENT (@ WAN - GATEWAY)
int ethernet1/2
   description ** Trunk @ WAN (MANAGEMENT VRF) **
   no shutdown
   duplex full
      switchport
      switchport trunk encapsulation dot1q
...
## 🔀 `IOS-SWITCH-MGMT` - (Switch Layer 3 - MANAGEMENT GATEWAY / L3 SVI) 
...
#                                                                                     #
#    MGMT         : 10.10.66.1/24 (V66 NETWORK GATEWAY)                               #
#    LOGIN        : admin / admin.cisco                                               #
#                                                                                     #
#    TRUNK:                                                                           #
#                                                                                     #
...
!# INTERFACE :: VLAN 66 @ MGMT

interface vlan 66
   description ** SVI-V66-MGMT <GATEWAY> **
   no shutdown
      ip address 10.10.66.1 255.255.255.0 
exit
!# Static default route @ WAN Interface (not neeeded)
!# ip route 0.0.0.0 0.0.0.0 10.66.1.1
