...
![image](https://github.com/user-attachments/assets/124ee939-17e2-4301-8306-17ad366d75da)
...
!#######################################################################################
!#  WAN INTERFACE + TRUNK TO SWITCH-L2                                                 #
!#######################################################################################
...
!# 3 - Create vPC Domain "666" and configure KeepAlive-Link (Primary = 200 (higher))
vpc domain 666
...
## 🔀 `IOS-SWITCH-WAN` - (Switch Layer 2 WAN) 
...
# -._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-._.-=-._.-=-._.-=-.#
#                                                                                     #
#    NEXUS        : IOS-SWITCH-WAN                                                    #
#    ROLE         : L2 SWITCH - WAN SWITCH                                            #
#                                                                                     #
#    MGMT         : 10.10.66.251/24                                                   #
#    LOGIN        : admin / admin.cisco                                               #
#                                                                                     #
#    TRUNK:                                                                           #
#                                                                                     #
...
!#######################################################################################
!#  MANAGEMENT SVI & INTERFACE (PHYSICAL INTERFACE IS E02, SAME LINK TO L3 SW)         #
!#######################################################################################
...
interface vlan 66
   ...
   no shutdown
      ip address 10.10.66.251 255.255.255.0 
exit
#! Default Gateway for mgmt @ MGMT switch
ip default-gateway 10.10.66.254

!#######################################################################################
!#   TRUNK INTERFACES (VRF: default) ==>> @ LAN                                        #
!#######################################################################################

!# INTERFACE :: ETH 0/0-1 @ SWITCH-L2 (Trunk) :: VRF default
int range ethernet0/0-1
   description ** Trunk @ VRF : default **
   no shutdown
   duplex full
      switchport
      switchport trunk encapsulation dot1q
...
!#######################################################################################
!#  TRUNK INTERFACES (VRF: management) ==>> @ SWITCH-MGMT                              #
!#######################################################################################

!# INTERFACE :: Eth 0/2 @ SWITCH-MGMT
interface ethernet 0/2
   description ** Trunk @ SWITCH-MGMT **
   no shutdown
   duplex full
      switchport
      switchport trunk encapsulation dot1q
...
!#######################################################################################
!#  TRUNK INTERFACES (VRF: management + default) ==>> @ MAGIC 66 + 10 @ WAN            #
!#######################################################################################

!# INTERFACE :: ETH 0/3 @ SWITCH-L2 (Trunk) :: VRF MANAGEMENT + DEFAULT (@ WAN - GATEWAY)
int ethernet0/3
   description ** Trunk @ WAN (MANAGEMENT + DEFAULT) **
   no shutdown
   duplex full
      switchport
      switchport trunk encapsulation dot1q
      switchport mode trunk
      switchport trunk allowed vlan 66,10
exit
...
interface range ethernet0/0-3,ethernet1/0-3,ethernet2/0-3
...
!#######################################################################################
!#  TRUNK INTERFACES (VRF: management) ==>> @ SWITCH-L2                                #
!#######################################################################################

!# INTERFACE :: Eth 2/0 @ SWITCH-L2
interface ethernet 2/0
   description ** Trunk @ SWITCH-L2 **
   no shutdown
   duplex full
      switchport
      switchport trunk encapsulation dot1q
...
!#######################################################################################
!#  L3 INTERFACE & SUB-INTERFACE @ LAN (VRFs: management & default)                    #
!#######################################################################################

#! L3 PHYSICAL INTERFACE @ GATEWAY FOR LAN (VRFs: management & default)
...
#! L3 SUB-INTERFACE @ GATEWAY FOR LAN (VRF: underlay / VLAN: 10)
interface Ethernet0/1.10
   description ** V10 Sub-Interface for -> IOS-SW-WAN **
   no shutdown
...
!#######################################################################################
!#  L3 INTERFACE @ WAN + NAT TRANSLATION / INTERNET (VRFs: management & default)       #
!#######################################################################################
...
! # ACL que define el tráfico a traducir (toda la red 10.10.x.x)
access-list Fz3r0-NAT-ACL permit 10.10.0.0 0.0.255.255

!# Overload all matching inside traffic to the WAN interface address
ip nat inside source list Fz3r0-NAT-ACL interface Ethernet 0/2 overload
