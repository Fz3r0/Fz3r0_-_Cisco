...
These are the mandatory steps to accomplish phase 2 of the underlay network confioguration:

1. `feature pim`: Enable PIM feature in Nexus switches. <br><br>
2. `ip pim sparse-mode`: Enable **ip pim sparse-mode** on ALL routed interfaces participating in the fabric (**physical** + **loopback0**).
    - This complement the OSPF unicast underlay by enabling multicast routing with PIM Sparse Mode. <br><br>
3. `Anycast RP` / `loopback1` _(Spines Only)_: Anycast RP (using both spines as RPs) provides High Availability (HA/Redundancy)
