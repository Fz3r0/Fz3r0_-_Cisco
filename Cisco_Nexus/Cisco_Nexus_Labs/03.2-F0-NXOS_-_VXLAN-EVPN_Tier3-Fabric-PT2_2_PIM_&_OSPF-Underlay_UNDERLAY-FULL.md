...
1. **`PIM Sparse-Mode`**: configured on **physical interfaces** and **loopback0**  
  → `ip pim sparse-mode` <br><br>
2. **`Rendezvous Point (RP)`**: **Static-RP** or **Anycast-RP**  
  → Static: `ip pim rp-address <Loopback0-IP(Spine-1)>`  
  → Anycast: `ip pim anycast-rp <loopback1-IP(Spine-2)> <loopback0-IP(Spine-1)>` <br><br>
4. Multicast group mapping per VNID  
5. VXLAN BUM replication mechanisms  
6. End-to-end flooding for unknown MAC addresses
...
- **`OSPF underlay`**: For **unicast routing**. All devices (BGWs, Spines, Leafs) participate in OSPF (UNDERLAY-F0). 
- **`/31`**: /31 P2P OSPF with point-to-point links between all tier Nexus Switches. **End-to-end unicast connectivity** validated (ping, traceroute, etc)
- **`Loopback 0`**: used as unique router ID and future **VTEP source IP (VTEP IP)**
...
### 2️⃣🛠️ Phase 2 Configuration Steps

1. `ip pim sparse-mode`: Enable **ip pim sparse-mode** on all routed interfaces (**physical** + **loopback0**).
    - This complement the OSPF unicast underlay by enabling multicast routing with PIM Sparse Mode. <br><br>
2. `loopback0 advertised in OSPF`: Used for **RPF (Reverse Path Forwarding)** & accesibility.
    - In the context of VXLAN, RPF is used to ensure that traffic is forwarded along the correct path, preventing loops and improving network stability. <br><br> 
3. `Anycast RP`: Anycast RP (using both spines as RPs) provides High Availability (HA/Redundancy)
   -  2 RPs (both Spines), using `loopback1` to set the Anycast IP (same IP for both Spines)  <br><br> 
4. Validate multicast operation using `ping`, `mroute`, or `IGMP` commands
...
PIM (Protocol Independent Multicast) is the multicast routing protocol used by VXLAN to handle BUM (Broadcast, Unknown unicast, and Multicast) traffic. In any VXLAN fabric, PIM will always be enabled in the underlay using PIM Sparse Mode.

Originally developed in the mid-1990s by Cisco and other industry contributors, PIM is defined in RFC 7761 (governed by IETF) and is vendor-neutral (supported by Cisco, Juniper, Arista, and others). It operates at Layer 3, independently of any specific unicast routing protocol (hence the name Protocol Independent).

- This protocol is activated per-interface, just like OSPF or other protocols, and allows multicast packets to be routed across the fabric in a scalable and efficient manner.

VXLAN encapsulates Layer 2 frames into Layer 3 IP packets and needs a way to replicate flooded traffic (BUM - Broadcast, Unknown unicast, and Multicast) across all VTEPs in the same segment (VNID- Virtual Network Identifier). To achieve this, VXLAN traditionally uses Multicast-based Flood-and-Learn, where each VNID is mapped to a unique multicast group IP. In this model, when one VTEP receives a BUM frame, it replicates it to a multicast group, and all other VTEPs listening to that group will receive the traffic.
...
- 🚫 This mode floods multicast traffic out **all interfaces**, regardless of whether there are receivers on the other end.
- It does not ask if there is interest, it just floods. Then, receivers respond and can join the multicast group.

**⚠️ This mode is NOT used in VXLAN deployments because it generates unnecessary traffic.**

### 2. Sparse-Mode 

- ✅ This is the mode used for VXLAN. Instead of flooding, a **Rendezvous Point (RP)** is defined (usually a Spine switch).
- When a Leaf wants to send multicast traffic (e.g., to group `239.1.1.1`), it doesn’t flood, it queries the RP to discover other receivers.
- The RP tracks all receivers and responds accordingly, forming the correct multicast distribution tree. 
...
### PIM Configuration: PIM on interfaces (supplicant)

Los pasos para habilitar PIM en las interfaces suplicantes, es decir, las que solicitaran informacion al RP (e.g. los leafs que solicitaran a los spines/rp). En el casi de este lab se habilitaran en ambos BGW, y en todos los Leafs (ninguno de los 2 spines)
...
4. Indicarle quien es el RP, agregando la IP loopback1 de los psines. 
...
### PIM Configuration: PIM on interfaces (RP / Spines)

Los pasos para habilitar PIM en las interfaces de los RP, es decir, los spines que envian la informacion a todos los suplicantes son basicamente lso mismos, con al diferencia que a los spines hay que agregar lienas adicionales para indicar que se esta usando rp anycast para el HA. esto se hace con el comando `ip pim anycast-rp 10.255.255.255 10.10.10.13` y `ip pim anycast-rp 10.255.255.255 10.10.10.14`. Es decir, solo cambia el paso 4 que sera Indicarle que el es un RP que comparte IP con otro RP, para eso se usan los loopback0 de ambos Spines

1. Habilitar el feature de PIM
2. Habilitarlo en las interfaces fisicas partcipantes con `ip pim sparse-mode`
3. Habilitarlo en la loopback0 con `ip pim sparse-mode`
4. Indicarle quien es el RP, agregando la IP loopback1 de los psines. 
5. **Indicarle que el es un RP que comparte IP con otro RP, para eso se usan los loopback0 de ambos Spines**
...
!# Step 5.  Indicarle que el es un RP que comparte IP con otro RP, para eso se usan los loopback0 de ambos Spines

!# Spine 1 - RP-1 (Loopback0)
ip pim anycast-rp 10.255.255.255 10.10.10.13
!# Spine 2 - RP-2 (Loopback0)
ip pim anycast-rp 10.255.255.255 10.10.10.14
...
#    - Lo1        : 10.255.255.255/32  (anycast RP)                                   #
#    - OSPF ID    : UNDERLAY-F0                                                       #
#    - OSPF AREA  : 0.0.0.0 (Backbone)                                                #
#                                                                                     #
#######################################################################################
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+  PIM - RP Anycast (RP Redundant) for PIM Sparse Mode (UNDERLAY - Multicast)         +
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
