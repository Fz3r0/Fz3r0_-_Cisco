...
While Part 1 focused on bringing up the **UNICAST UNDERLAY** with OSPF, now in Lab 2, we complete the underlay network with the **UNICAST UNDERLAY**, by introducing `Multicast` support using `PIM Sparse Mode`, a key requirement to support `VXLAN BUM (Broadcast, Unknown Unicast, and Multicast)` traffic. Key Concepts Introduced in This Lab will be: PIM Sparse Mode (ip pim sparse-mode), RP assignment (static or anycast-RP), Multicast group mapping, VXLAN BUM traffic handling, End-to-end flooding for unknown MACs

## 🧠 Underlay Components and Phases for configuration:

✅ Phase 1: OSPF Underlay (Already Done in Lab 1)

- All devices (BGWs, Spines, Leafs) participate in OSPF (UNDERLAY-F0)
- Loopback0 used as router ID and future VTEP source IP
- /31 P2P links between all tier Nexus Switches
- End-to-end unicast connectivity validated (ping, traceroute, etc)
- Extra configs (ssh, hostnames, vPC, WAN, etc)

🔥 Phase 2: PIM Sparse Mode (This Lab)

- All routed interfaces join PIM Sparse Mode
...
### 🧱 UNDERLAY Breakdown in 2 Phases

To build a reliable VXLAN underlay, you need two core components:

1. ✅ Dynamic Routing for Unicast - OSPF takes care of routing all unicast traffic across the fabric. <br><br>
    - We use OSPF with point-to-point /31 links between devices
    - Loopback0s as unique router IDs (and later as VTEP IPs).
    - This provides stable unicast IP routing between all Leafs, Spines, and BGWs.

2. ✅ Multicast Routing for BUM Traffic - PIM enables VXLAN to replicate BUM traffic using multicast groups. <br><br>
    - complement the underlay by enabling multicast routing with PIM Sparse Mode.
    - This is necessary for VXLAN to handle unknown or broadcast traffic using multicast flooding mechanisms.
...
VXLAN encapsulates Layer 2 frames into Layer 3 IP packets and needs a way to replicate flooded traffic (BUM) across all VTEPs in the same segment (VNID). To achieve this, VXLAN traditionally uses Multicast-based Flood-and-Learn, where each VNID is mapped to a unique multicast group IP.
...
2. Multicast routing between all switches — handled by PIM + RPF rules
...
## 🧠 Summary of What We’ll Do in This Lab
...
- Configure a static RP — for now, we’ll pick SPINE-1 as the RP.
