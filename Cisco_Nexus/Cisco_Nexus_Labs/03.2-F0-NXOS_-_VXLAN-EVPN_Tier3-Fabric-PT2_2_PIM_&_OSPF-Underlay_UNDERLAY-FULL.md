...
1. Enable the `pim` **feature**.
2. Configure **PIM Sparse-Mode on ALL routed physical interfaces** `Eth1/x`.
3. **Enable PIM** on `loopback0`.
4. **Point to the RP** by specifying the `loopback1` IP address shared by the Spines (Anycast RP).
