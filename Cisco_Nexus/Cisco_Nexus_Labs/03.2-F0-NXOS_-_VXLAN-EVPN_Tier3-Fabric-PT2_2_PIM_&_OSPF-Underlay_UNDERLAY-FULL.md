...
1. **`PIM Sparse-Mode`**: configured on **physical interfaces** and **loopback0**  
  → `ip pim sparse-mode`
2. **`Rendezvous Point (RP)`**: Static-RP or Anycast-RP (I will be using Anycast on both spines for HA redundancy) - `ip pim anycast-rp {IP loopback 1} {IP loopback 0 / RID}`
  → **Static-RP** or **Anycast-RP** (*we’ll use Anycast-RP across both Spines for HA redundancy*)  
  → `ip pim anycast-rp {loopback1 IP} {loopback0 IP / RID}`
3. Multicast group mapping per VNID  
4. VXLAN BUM replication mechanisms  
5. End-to-end flooding for unknown MAC addresses
...
- **OSPF underlay** (`UNDERLAY-F0`) for **unicast routing**. All devices (BGWs, Spines, Leafs) participate in OSPF (UNDERLAY-F0). 
- **`/31`** P2P OSPF with point-to-point links between all tier Nexus Switches. **End-to-end unicast connectivity** validated (ping, traceroute, etc)
- **`Loopback 0`** used as unique router ID and future **`VTEP source IP (VTEP IP)`**
- Optional extra configs (ssh, hostnames, vPC, VRF, WAN/NAT, etc)
...
1. Enable `ip pim sparse-mode` on all routed interfaces (physical + `loopback0`). This complement the OSPF unicast underlay by enabling multicast routing with PIM Sparse Mode.  
2. Advertise `loopback0` in OSPF to be used for **RPF (Reverse Path Forwarding)**. In the context of VXLAN, RPF is used to ensure that traffic is forwarded along the correct path, preventing loops and improving network stability  
3. Configure RP:
   - `Static RP`: Just 1 RP (Spine-1)
   - `Anycast RP`: 2 RPs, using `loopback1` on both Spines for HA (used in this lab)
4. (Optional) Validate multicast operation using `ping`, `mroute`, or `IGMP` commands
