...
1. Enable `ip pim sparse-mode` on all routed interfaces (physical + `loopback0`).
    - This complement the OSPF unicast underlay by enabling multicast routing with PIM Sparse Mode. <br><br>
2. Advertise `loopback0` in OSPF to be used for **RPF (Reverse Path Forwarding)**.
...
3. Configure RP:
   - `Anycast RP`: 2 RPs, using `loopback1` on both Spines for HA (used in this lab)  <br><br> 
5. (Optional) Validate multicast operation using `ping`, `mroute`, or `IGMP` commands
