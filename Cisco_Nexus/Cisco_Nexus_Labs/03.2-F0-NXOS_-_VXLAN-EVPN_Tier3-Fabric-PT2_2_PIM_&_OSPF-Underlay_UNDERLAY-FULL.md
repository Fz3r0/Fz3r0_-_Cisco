...
Now, in Lab 2, we complete the underlay network by introducing `Multicast` support using `PIM Sparse Mode`, a key requirement to support `VXLAN BUM (Broadcast, Unknown Unicast, and Multicast)` traffic.
...
## 🧠 Underlay Components: Now Fully Complete
...
1. Dynamic Routing for Unicast
We use OSPF with point-to-point /31 links between devices, plus Loopback0s as unique router IDs (and later as VTEP IPs). This provides stable unicast IP routing between all Leafs, Spines, and BGWs.

✅ OSPF takes care of routing all unicast traffic across the fabric.

2. Multicast Routing for BUM Traffic
Now we complement the underlay by enabling multicast routing with PIM Sparse Mode. This is necessary for VXLAN to handle unknown or broadcast traffic using multicast flooding mechanisms.

✅ PIM enables VXLAN to replicate BUM traffic using multicast groups.
...
🧪 Key Concepts Introduced in This Lab
