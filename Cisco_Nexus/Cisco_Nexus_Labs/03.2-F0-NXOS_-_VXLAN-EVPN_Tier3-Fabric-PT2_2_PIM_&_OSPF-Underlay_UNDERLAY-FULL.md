...
- PIM Sparse Mode: configured at **physical interfaces** and **loopback0** - `ip pim sparse-mode`
- Rendezvous Point (RP) assignment: Static-RP or Anycast-RP (I will be using Anycast on both spines for HA redundancy) - `ip pim anycast-rp {IP loopback 1} {IP loopback 0 / RID}`
...
To build a reliable VXLAN underlay, you need two core components y de la misma manera el lab fue dividido em ambas fases:
...
1. All routed interfaces (physical `eth1/x` & `loopback0`) join `PIM Sparse Mode`, this complement the OSPF unicast underlay by enabling multicast routing with PIM Sparse Mode.
2. `Loopback0` is advertised in OSPF to be used as `RPF check source`.
3. One of the Spines is configured as the Rendezvous Point (RP) (for `static RP`) //or// Both of the Spines are configured as the Rendezvous Points (RPs) (for `Anycast RP`), we will use this option in this lab to have HA redundancy. 
