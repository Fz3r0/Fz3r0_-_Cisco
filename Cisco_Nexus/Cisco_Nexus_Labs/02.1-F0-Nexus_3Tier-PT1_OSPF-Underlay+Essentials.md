...
ip route 8.8.8.8 255.255.255.255 123.1.1.1
ip route 8.8.8.8 255.255.255.255 100.1.5.2 250
...
ip route 8.8.8.8 255.255.255.255 123.2.2.1
ip route 8.8.8.8 255.255.255.255 100.1.5.1 250
...
exit
!# Default Route @ Internet (A)
ip route 0.0.0.0 0.0.0.0 123.3.3.1
...
exit
!# Default Route @ Internet (B)
ip route 0.0.0.0 0.0.0.0 123.4.4.1
...
## 🌎 `INTERNET` - (Internet Circuit Simulation)
...
! # Habilita el reenvío de paquetes IP entre interfaces, necesario para que el router actúe como un verdadero router y no solo como un host con múltiples interfaces.
ip routing
