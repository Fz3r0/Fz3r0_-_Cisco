...
This lab represents the first phase of a 4-part VXLAN/EVPN deployment using a 3 tier data center architecture (core, distribution y access), pero tambien puede servir en una tier 2 leaf & spine. The main goal de este primer laboratiorio is to build a solid and scalable topology foundation, with a fully functional **underlay network** using a dynamic link state routing protocol like ISIS or OSPF (in this case I will use OSPF).

### VXLAN

Se encarga de Crear un dominio de Broadcast (es decir como si estuviera en Layer 2) que a travez de conexiones por tuneles puede ser compartido entre multiplces switrches par autilizando layer 3! es decir sin usar spaning tree ni nada de eso (transportar capa 2 por capa 3). 

- eL TRAIFCO CAPA 2 SE PASA SIN SER TOCADO, TODO TUNELEADO, LOS DISPOSITIVOS QUE MANDAN ESE TRAFICO NO TIENEN NI IDEA QUE ENTRARAN A UNA VXLAN, PARA ELLOS ES CUALQUIER TRAIFCO CAPA 2 DE RED TRADICIONAL. ES DECIR, LA MAC DE ORIGIN Y DESTINO NO CAMBIARA

Por ejemplo, si yo creo en LEAF-1 la VxLAN "5100" en realidad lo que estouy creando es una tabla de direcciones mac en esa tabla con ID "5100", todos los demas dispositivos que se "sepan" esa tabla vXLAN 5100 podran desencampuslar el trafico. A estos dispositivos que guardan las tablas (siempre son los bordes como Leafs......o BGWs??? no se si los bgw tmb...)

### 🛤️ VTEP: 

Lapuntan hacia lso servidores de acceso que estan en layer 2, (validar si los BGW tmbien son VTEP vektor?) explica lo que es un VTEP... 

Los VTEP crean tuneles entre si para mandar el trafrico L2 entre si de punto A a punto B, ejemplo de LEAF1 al Leaf 2, o del Leaf 2 al BGW1

Del otro lado de los VTEP en los leafs usuarlemnte hyay servidores que envian trunks, quizas una VM riel, CUSM, DNA Center, AD, WLC, etc

Todos esos servers se siguen comuicando en capa 2 como un cualqueir red tradicional LAN, el chiste dle VTEP es que los LEAFs se peudan "aprender" esas VLANs capa 2, y encapsularlas capa 3 por ocnexiones por tuneles, a eso lo llamamos vxlan bla bla. 

### 🛤️ VXLAN: `Underlay Network`
...
**En este Lab me voy a enfocar en la primer fase del underlay y crear un link soliudo de OSPF entre los equipos, también aprovechare pare crear los vPC y una slaida a internet tanto para la VRF de management como la default, es decir, una base completamente solida para la fase 1 de3l underlay, incluyendo las demas configuraicones secudnarias como nombres, ssh, disvoery, bpvs, etc, etc. This setup ensures that each node can reach any other at Layer 3, which is a core requirement for VXLAN overlays.**

En el siguiente laboratiorio me enfocaré en la fase 2 del underlay que es habilitar multicast y PIM y Once this is in place, I will focus on enabling VXLAN in its raw form, to understand how encapsulation and VNI-based forwarding works without the help of EVPN. Finally, I will introduce EVPN control plane, which automates MAC/IP learning, scales the overlay, and brings the full power of modern data center fabrics. 
